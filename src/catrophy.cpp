...
////////////////////////////////////////////////////////
// THIS IS THE CLASS WE WANT TO UPDATE AND FEED TO NN //
////////////////////////////////////////////////////////
class gameState{
public:
  #define USE_GFXnej
  #define TRACK_TIME
  #define TRACK_TIME2
  #define DEBUG_GAME_STATEinte

  #define GRID_SIZE 13
  #define GRID_N (GRID_SIZE-1)/2
  #define GRID_HORIZON 1500
  #define GRID_STEP (GRID_HORIZON/GRID_N)

  #define MAGIC_SPEED 100

  float grid[GRID_SIZE][GRID_SIZE];
  float stuff_grid[GRID_SIZE][GRID_SIZE];
...
CATrophy::runMenu()
{
    ...
    //WE ARE IN A HURRY, SO WE START STRAIGHT AWAY!
    startNewGame();

    //ORIGINAL CODE DOES WHATS BELOW!
    do
    {
...
CATrophy::startNewGame()
{
    ...
    m_nbTurns = 0;
    difficulty = Difficulty (initialState::difficulty);
    player[0]->setName( "RacerKnockers" );
    player[0]->setColor(HSVColor( (int)initialState::color,0,0 ));
    //THEN
    gameLoop();
...
CATrophy::run()
{
  ...
  ///////////////////////////////////////////////////////////////
    ...
    int framesThisSecondResetThing=0;
            ...
            (!raceOver || CL_System::get_time()-raceOverTime<3000 ) )
    {
        ...
        //int gameTime=(CL_System::get_time()-gameStartTime)*initialState::magicSpeed;
        int gameTime=(CL_System::get_time()-gameStartTime);

        #ifdef TRACK_TIME2
          if(CL_System::get_time()-framesThisSecondResetThing>=1000)
          {
            framesThisSecondResetThing=CL_System::get_time();
            std::cout << framesThisSecond << std::endl;
            framesThisSecond = 1;
          }
          else
          {
        ...
        {
            ///////////////////////////////////////////////
            // UPDATE GAMESTATE SO WE KNOW HOW TO DRIVE! //
            ///////////////////////////////////////////////

            for(int i=0;i<GRID_SIZE;i++)
              for(int j=0;j<GRID_SIZE;j++)
                state.stuff_grid[i][j]=0;

            state.bullets=m_RacePlayer[0]->getBullets();
            state.life=m_RacePlayer[0]->getLife();
            state.boost=m_RacePlayer[0]->getTurbo();
            state.velocity=m_RacePlayer[0]->getSpeed();
            state.direction=m_RacePlayer[0]->getDirection();
            state.x=m_RacePlayer[0]->getX();
            state.y=m_RacePlayer[0]->getY();

            glm::vec2 pos = glm::vec2(state.x,state.y);

            float d = state.direction*(PI/180);
            glm::vec2 forward = glm::vec2( cos(d),sin(d) );
            glm::vec2 left = glm::vec2( sin(d),-cos(d) );
            glm::vec2 temp,temp2;
            int tx,ty;

            for(int i=-GRID_N ; i<=GRID_N ; i++ ){
              for(int j=-GRID_N ; j<=GRID_N ; j++ ){
                temp=pos+(float)(i*GRID_STEP)*forward+(float)(j*GRID_STEP)*left;
                  state.grid[i+GRID_N][j+GRID_N] = getSpeedLimit(temp.x,temp.y);

              }
            }

            //PROJECT ALL OBJECTS INTO GRID:
            for( unsigned int gt=0; gt<goody.size(); gt++ ){
              for( unsigned int gi=0; gi<goody[gt].size(); gi++ ){
                temp=glm::vec2(goody[gt][gi]->getX(),goody[gt][gi]->getY())-pos;
                temp2=glm::vec2(dot(temp,forward),dot(temp,left));

                  if(temp.x>GRID_HORIZON+GRID_STEP/2 || temp.y>GRID_HORIZON+GRID_STEP/2)
                    continue;

                tx=(int)(temp2.x/GRID_STEP+0.5)+GRID_N;
                ty=(int)(temp2.y/GRID_STEP+0.5)+GRID_N;
                tx = tx > GRID_SIZE ? GRID_SIZE-1 : tx;
                ty = ty > GRID_SIZE ? GRID_SIZE-1 : ty;
                tx = tx < 0 ? 0 : tx;
                ty = ty < 0 ? 0 : ty;
                //std::cout << "[ " << tx << " " << ty << " ]" << std::endl;
                state.stuff_grid[tx][ty]=gt+1;
              }
            }

            for(c=1; c<CA_RACEMAXPLAYERS; ++c){
              temp=glm::vec2(m_RacePlayer[c]->getX(),m_RacePlayer[c]->getY())-pos;
              temp2=glm::vec2(dot(temp,forward),dot(temp,left));
              ...
              ty = ty < 0 ? 0 : ty;
              //std::cout << "[ " << tx << " " << ty << " ]" << std::endl;
              state.stuff_grid[tx][ty]=8;
            }

            #ifdef TRACK_TIME
              double old_time;
              std::cout << (double)CL_System::get_time()-old_time << std::endl;
              old_time=(double)CL_System::get_time();
            #endif
            ...
            for( c=0; c<CA_RACEMAXPLAYERS; ++c )
            {
                ...
                {       // Check if first player has finished
                    ...
                    if(!firstPlayerFinished)
                      std::cout << "First player finnished at T=" << gameTime << "ms" << std::endl;
                  #endif
            ...
            if(raceOver && raceOverTime==0) raceOverTime = gameTime;
            ...
            // Place goodies:
            //
            if( gameTime > (unsigned int)(goodyTime+CA_GOODYTIME) )
            {
                goodyTime = gameTime;
                placeGoody();
            }
       ...
       if( debug && trackInfo ) // TODO: trackInfo should be in Track
        ...
        //
        if( lightState!=3 && (CL_System::get_time()-gameStartTime) > 0 )
        {
        ...
        //
        if( !allowShooting && (gameTime-gameStartTime) > 3000 )
        {
            allowShooting=true;
        }
...
CATrophy::measureFrameTime( bool start ) {
    ...
    {
        int timeElapsed = (CL_System::get_time()-frameStart)*MAGIC_SPEED;
        if( timeElapsed>0 ){
            //framesPerSec = 1000.0 / (timeElapsed*initialState::magicSpeed);
            framesPerSec = 1000.0 / (timeElapsed);
        }
